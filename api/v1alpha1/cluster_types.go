/*
Copyright 2021 RadonDB Organization.
...
type ClusterSpec struct {
	...
	// +optional gry: 这两行注释是copy,下面两行是自己加的,确认下这么写是不是写入yaml后就会显示,参考presslabs SecretName枚举
...
// gry: 这里面主要是从helm原有的开放的可编辑参数构成,就是多了个资源占用默认值
// gry(TODO)这里为什么没有mysql的image参数呢？跟其它的不一致，也跟helm中values.yaml的不一致,放哪里了呢
...
type MysqlOpts struct {
	...
	// gry: 借用了apis/mysql/v1alpha1/mysqlcluster_types.go里面的写法
	// A map[string]string that will be passed to my.cnf file.
	// +optional
	MysqlConf MysqlConf `json:"mysqlConf,omitempty"`

	// gry: 借用了apis/mysql/v1alpha1/mysqlcluster_types.go里面的写法,搜"core.ResourceRequirements"
	// gry: 对应helm的values.yaml中resources: {}
	// gry: 相当于直接设置默认值，在presslabs里面是set:setPodSpecDefaults()
...
// gry: 参考apis/mysql/v1alpha1/mysqlcluster_types.go的PodSpec定义,结合自己的
...
// gry(TODO):跟presslabs的枚举值不一样，为什么？什么是condition type?
