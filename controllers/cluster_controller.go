...
import (
	...
	// gry(TODO):要调整目录么??建议移动到指定库下面,不然以后没掉了会很尴尬
...
func (r *ClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	// gry: 这个函数presslabs放到了newReconciler()里,做了封装
	// gry(TODO):这里log注册名要改
	...
	defer func() {
		if !reflect.DeepEqual(status, instance.Status) {
			// gry(TODO):更新哪个跟哪个?new出来的实例?后续每次调谐都要new??
...
func (r *ClusterReconciler) SetupWithManager(mgr ctrl.Manager) error {
	// gry: 对应mysqlcluster_controller.go add(),presslabs做了封装.
